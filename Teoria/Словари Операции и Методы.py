...
t=dict(a)  #{'moskva': 495, 'piter': 812, 'penza': 8412}
...
a = ['a', 'b', 'c']
b = [1, 2, 3]
d = { i_a: i_b for i_a, i_b in zip(a, b)}
#4) Четвертый способ:
...
l = s.split() #после сплита всегда из строки получается список
print(l)   #['IVANOV', 'IVAN', 'Samara', 'SGU', '5', '4', '5', '5', '5', '4', '3']
person['lastName'] = l[0]
person['firstName'] = l[1]       #подбираем к ключам значения из списка по индексам
person['city'] = l[3]
person['university'] = l[3]  #значения могут быть не уникальными
person['marks'] = []  #здесь пустое значение


print(person)   #{'lastName': 'IVANOV', 'firstName': 'IVAN', 'city': 'SGU', 'university': 'SGU', 'marks': []}

for i in l[4:]:  #здесь ключу 'marks' присваиваем срез значений списка в виде списка
    person['marks'].append(i)
print(person)
#   {'lastName': 'IVANOV', 'firstName': 'IVAN', 'city': 'SGU', 'university': 'SGU', 'marks': ['5', '4', '5', '5', '5', '4', '3']}
#8) операции:
...
# добавляем еще одно значение к ключу в словаре
names_dict ={'А': ['Алина'], 'Б': ['Бибочка'], 'И': ['Иван'],
             'М': ['Марина', 'Мария'], 'П': ['Петр', 'Петр']}
# names_dict['И'] += ['Илья']
names_dict['И'] = names_dict['И'] + {'Илья'}
...
#   names_dict_list[first_letter] = names_dict_list.setdefault(first_letter, []) + [name]
